...
@section('content')
    <h1><a class="block nt-1 text-lg leading-tight font-medium text-black hover:underline">Bienvenido a la seccion de usuarios</a></h1>
				...
				<div class="md:flex">
					<div class="p-8">
						<div class="uppercase tracking-wide text-sm text-indigo-500 font-semibold">{{$usuario->name}}</div>
						    ...
						    <a href="/usuario/{{$usuario->id}}" class="block nt-1 text-lg leading-tight font-medium text-black hover:underline">Ver mas...{{$usuario->id}}</a>
					</div>
				</div>
			</div>
		@endforeach
